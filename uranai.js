...
class DialogueSystem{
    constructor(orderbox_logined,orderbox_unlogined,speech,btn1,btn2,entry){
        ...
        this.show_log_4debug_flg = "do";//judge_idにおいて、show_log_4debug関数が上手く動かないので、泣く泣く追加したフラグです。くぅーーー
    ...
    show_log_4debug(){//console.logに表示させるための機能
        ...
        if(this.show_log_4debug_flg == "do"){
            //############パックのテーブル表示#################
            ...
            var font_list = [`color:black;font-weight:normal`,//非太字、黒色にする記述
                            `color:red;font-weight:bold`//太字、赤色にする記述
                            ]
            console.log(`会話パック変更フラグ: ${this.dialogue_changed_flg}`)

            /** ===========  会話についてのログ製作 ===============**/
            if(this.dialogue_changed_flg==1 ){//flg 1=会話変更、 2=命令実行後に会話変更予定、 0=会話変更なし
            ...
            【%c会話%c: ${message}】`,
                ...
                `font-weight:bold;`,`font-weight:normal;`,font_list[order_logfont_pattern[0]],font_list[order_logfont_pattern[1]],font_list[order_logfont_pattern[2]],font_list[order_logfont_pattern[3]],font_list[order_logfont_pattern[4]],font_list[order_logfont_pattern[5]],font_list[order_logfont_pattern[6]],font_list[order_logfont_pattern[7]],
                `font-weight:bold;`,`font-weight:normal;`,font_list[dialogue_logfont_pattern[0]],font_list[dialogue_logfont_pattern[1]],font_list[dialogue_logfont_pattern[2]],font_list[dialogue_logfont_pattern[3]],font_list[dialogue_logfont_pattern[4]],font_list[dialogue_logfont_pattern[5]],
            );
    ...
    link(){//≪データ記入場≫
        ...
        var user_data = [
            ...
            this.dialogue_changed_flg,  // 2:会話パック変更フラグ
            this.order_changed_flg      // 3:命令パック変更フラグ
        ...
        get(dbRef).then((snapshot) => {//page2, letter.indexの時のデータ取り出し
            ...
            if (snapshot.exists()) {//パスワードが登録されていた場合
                ...
                try{
                    ...
                    this.dialogue_changed_flg = this.user_savedata[2] // 2:会話パック変更フラグ
                    this.order_changed_flg = this.user_savedata[3]    // 3:命令パック変更フラグ
    ...
    judge_id(){//id判定。
        ...
        this.show_log_4debug_flg = false;
        ...
        get(dbRef).then((snapshot) => {//page2, letter.indexの時のデータ取り出し
            ...
            this.show_log_4debug_flg = "do";
            this.show_log_4debug();
        });

        this.show_log_4debug_flg = "skip";
    }
